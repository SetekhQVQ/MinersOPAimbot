...
void Hooks::InitHooks()
{
    ...
    HOOKATTACH(LightBase_UpdatePriority);
    HOOKATTACH(EnemyScanner_UpdateTagProgress);
...
void Hooks::hkLightBase_UpdatePriority(app::LightBase* __this, app::Vector3 cameraPosition, app::Plane__Array* cameraFrustum, MethodInfo* method)
{
#ifdef USE_DETOURS
    app::LightBase_UpdatePriority(__this, cameraPosition, cameraFrustum, method);
#else
    static auto fpOFunc = reinterpret_cast<void (*)(app::LightBase*, app::Vector3, app::Plane__Array*, MethodInfo*)>(fpMap["LightBase_UpdatePriority"]);
    fpOFunc(__this, method);
#endif // USE_DETOURS

    if (Player::fullBrightToggleKey.isToggled())
    {
        __this->fields.Intensity = 1.0f;
        __this->fields.Physical = 1.0f;
        __this->fields.Range = 160.0f;
        __this->fields.Color = {0.5f, 0.5f, 0.5f, 1.0f};
    }
}

void Hooks::hkEnemyScanner_UpdateTagProgress(app::EnemyScanner* __this, int32_t maxTags, MethodInfo* method)
{
#ifdef USE_DETOURS
    app::EnemyScanner_UpdateTagProgress(__this, maxTags, method);
#else
    static auto fpOFunc = reinterpret_cast<void (*)(app::EnemyScanner*, int32_t, MethodInfo*)>(fpMap["EnemyScanner_UpdateTagProgress"]);
    fpOFunc(__this, method);
#endif // USE_DETOURS

    if (Player::quickBioToggleKey.isToggled())
    {
        __this->fields.m_rechargeDuration = 0.0f;
    }
}
...
HRESULT __stdcall Hooks::hkPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
    if (!G::initialized)
    {
        if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D11Device), (void**)&G::pDevice)))
        {
            ...
            G::defaultFont = io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\SimHei.ttf", 15.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());
            ImFontConfig config;
            config.FontBuilderFlags |= ImGuiFreeTypeBuilderFlags::ImGuiFreeTypeBuilderFlags_ForceAutoHint;
            G::espFont = io.Fonts->AddFontFromFileTTF("c:\\Windows\\Fonts\\SimHei.ttf", 15.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());
